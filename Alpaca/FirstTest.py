...
bars = api.get_bars(SYMBOLS, TimeFrame.Minute, yesterday.isoformat(), today.isoformat()).df
bars.index = bars.index.tz_convert('US/Mountain')

#Get various moving averages:
moving_average_windows=[5,9,20,50,200]
moving_average_keys=[f'{window_size}_sma' for window_size in moving_average_windows]

for symbol in bars.symbol.unique():
	bars_symbol = bars[bars["symbol"]==symbol]
	for window_size,window_key in zip(moving_average_windows,moving_average_keys):
		bars.loc[bars.symbol==symbol,window_key]=ta.trend.sma_indicator(bars_symbol["close"],window=window_size)
